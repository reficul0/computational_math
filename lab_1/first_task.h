...
{
	// ћетод итераций.
	// »сходное уравнение: 4x + e^x = 0
	// »нтервал: [a;b] = [-1; 1]
	// 
	// ѕриводим к виду x=g(x):
	// x = -(e^x)/4 = g(x)
	// ¬ычисл€ем x0:
	// x0 = -(e^0)/4 = -1/4
	// ѕерва€ итераци€:
	...
	/**
	 * \brief найти корень выражени€ 4x + e^x = 0
	 * \param interval [a;b], где a-first, b-second
	 * \param eps точность
	 * \param g_x функци€, которой сведена исходна€ формула x=g(x)
	...
	) throw(std::runtime_error)
	{
		...
		while (true)
		{
			...
			if (x_next > interval.second)
				throw std::runtime_error(
					"»скомый корень не принадлежит интервалу: [" + std::to_string(interval.first)
					+ "; " + std::to_string(interval.second) + "]"
				);

			// ≈сли нужна€ точность достигнута ..
			...
			if (iterations > 10000)
				throw std::runtime_error(
					"ќбнаружено зависание, слишком большое количество итераций: " + std::to_string(iterations)
	...
	/**
	 * \brief ѕроверить условие сходимости
	...
	) throw(std::runtime_error)
	{
		auto check = [&derivative_g_x](double x)
		{
			if (abs(derivative_g_x(x)) > 1)
				throw std::runtime_error("g`(x) не удовлетвор€ет условию сходимости в точке: " + std::to_string(x));
		};
